...
#-----------------------------
  ...
  site_scores <- as.data.frame(scores(pca,scaling=site.scaling,display="sites",choices=axes))
  ...
  site_scores <- cbind(site_scores, metadata)
  if(reverse.x){
    site_scores[1] <- site_scores[1] * (-1)
  }
  if(reverse.y){
    site_scores[2] <- site_scores[2] * (-1)
  }
  # Retrieve the site's scores in scaling 2 for the left plot and bind it to the metadata
  site_scores2 <- cbind(as.data.frame(scores(pca, scaling= species.scaling, display="sites",choices=axes)), metadata)
  if(reverse.x){
    site_scores2[1] <- site_scores2[1] * (-1)
  }
  if(reverse.y){
    site_scores2[2] <- site_scores2[2] * (-1)
  }

  # Retrieve species' scores (Scaling 2)
  sp_scores <- as.data.frame(scores(pca,scaling = species.scaling, display="species", choices=axes))
  
  if(reverse.x){
    sp_scores[1] <- sp_scores[1] * (-1)
  }
  if(reverse.y){
    sp_scores[2] <- sp_scores[2] * (-1)
  }
...
#-----------------------------
  ...
  if(is.null(second.group)){
    ...
    p_sites <- p_sites + geom_point(data=site_scores, aes_string(x = axes_name[1], y = axes_name[2], fill = main.group, colour = main.group, shape=second.group), alpha= p.alpha, size = point.size)
    }
  ...
  # Custmize colour of fill if provided
  if(!is.null(scale.colour)){
    p_sites <- p_sites + scale_colour_manual(values=scale.colour)
  }
  ...
  if(is.null(labels) & is.null(second.group)){
    p_sites <- p_sites + geom_label(data = label_pos, aes_string(x = "x", y = "y", label = main.group, fill = main.group), alpha= 1, fontface="bold", size = font.size, inherit.aes = FALSE)
  }
  # If there are labels provided and no second group
  if(!is.null(labels) & is.null(second.group)){
    p_sites <- p_sites + geom_label(data = label_pos, aes_string(x = "x", y = "y", label = "labels", fill = main.group), alpha= 1, fontface="bold", size = font.size, inherit.aes = FALSE)
  }
  # If there are no labels provided and a second grouping variable
  if(is.null(labels) & !is.null(second.group)){
    p_sites <- p_sites + geom_label(data = label_pos, aes_string(x = "x", y = "y", label = main.group, fill = main.group, colour = main.group), alpha= 1, fontface="bold", colour = "black", size = font.size, inherit.aes = FALSE)+ scale_colour_manual(values=scale.colour)+
      scale_fill_manual(values=scale.fill)
  }
  # If there are labels provided and a second grouping variable
  if(!is.null(labels) & !is.null(second.group)){
    p_sites <- p_sites + geom_label(data = label_pos, aes_string(x = "x", y = "y", label = "labels", fill = main.group, colour = main.group), alpha= 1, fontface="bold", colour = "black", size = font.size, inherit.aes = FALSE)
  }
}
  p_sites <- p_sites + theme(axis.title = element_text(size = axis.size, face = "bold"), axis.text = element_text(size = axis.text), panel.background = element_blank())
...
#-----------------------------
  ...
  if(is.null(second.group)){
     p_sp <- p_sp + geom_point(data = site_scores2, aes_string(x = axes_name[1], y = axes_name[2], fill = main.group, colour = main.group), alpha= p.alpha, size = point.size)
	  ...
	  p_sp <- p_sp + geom_point(data=site_scores2, aes_string(x = axes_name[1], y = axes_name[2], fill = main.group, colour = main.group, shape=second.group), alpha= p.alpha)
  }
  ...
  p_sp <- p_sp + geom_point(data=sp_scores[sp_scores$fit > goodness.thresh,], aes_string(x = axes_name[1], y = axes_name[2]),shape=21,size= point.size)
  ...
  p_sp <- p_sp + theme(axis.title = element_text(size = axis.size, face = "bold"), axis.text = element_text(size = axis.text), panel.background = element_blank())
...
#-----------------------------
  ...
  # Final plots
  #------------
  g <- grid.arrange(p_sites,p_sp, ncol=2, nrow = 1, widths =  c(.5,.5))
  ggdraw(g)+theme_void()
}
